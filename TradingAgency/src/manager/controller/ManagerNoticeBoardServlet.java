 ...
 */
@WebServlet(name = "ManagerNoticeBoard", urlPatterns = { "/managerNoticeBoard" }) //자유게시판 페이징처리하는 서블릿
public class ManagerNoticeBoardServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("currentPage")==null)currentPage=1; //첫번째 페이지면 1로 설정
		else currentPage = Integer.parseInt(request.getParameter("currentPage"));//1페이지가 아니면 그 페이지 값을 가져옴.
		
		ManagerBoardPageData mbpd = new ManagerBoardService().freeBoardSelect(currentPage);
		}
