...
//				String userId = (((Member)session.getAttribute("user")).getUserId());
				...
				for(int i = 1; i<6; i++) {
				String beforeFileName = multi.getOriginalFileName("upfile"+i);

//				String beforeFileName = multi.getOriginalFileName("upfile5"); // 원본파일
				System.out.println("beforeFileName"+beforeFileName);
				
				String afterFileName = multi.getFilesystemName("upfile"+i); //바뀐파일
