...
/**
 * Servlet implementation class ProductTempInsertServlet
 */
@WebServlet(name = "ProductTempInsert", urlPatterns = { "/productTempInsert" })
public class ProductTempInsertServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ProductTempInsertServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 파일 업로드를 구현하려면 몇가지 정보가 필요함
				...
				//객체에 값 저장
				ImageFile imgFile = new ImageFile(beforeFileName,afterFileName, fullFilePath,fileSize,userId, uploadTime);
				
				int result = new ProductService().uploadFile(imgFile);		
				if(result>0)
				{
